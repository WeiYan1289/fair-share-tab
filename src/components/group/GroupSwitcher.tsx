    ...
    <div ref={rootRef} className="relative">
      ...
      {open && (
        <>
          ...
          <div className="fixed inset-x-0 bottom-0 z-50 rounded-t-xl bg-white p-3 pb-6 shadow-[0_-20px_40px_rgba(19,46,40,0.2)] sm:absolute sm:inset-x-auto sm:top-full sm:right-0 sm:bottom-auto sm:z-20 sm:mt-2 sm:w-80 sm:rounded-lg sm:p-2.5 sm:shadow-[0_24px_48px_-16px_rgba(19,46,40,0.28)]">
              ...
              {groups.map((g) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="text-[13.5px] font-bold text-ink">{g.groupName}</p>
                    <p className="text-[11px] text-muted-2">
                      {g.memberCount} member{g.memberCount === 1 ? "" : "s"}
                    </p>
                  </div>
                  {g.groupId === currentGroupId && <span className="text-emerald">✓</span>}
                </button>
              ))}
            </div>
            ...
            <button
              ...
              className="flex w-full items-center gap-2.5 rounded-md border border-dashed border-ink/18 px-3 py-2.5 text-left text-[13px] font-bold text-muted"
            >
              + Create new group
            </button>
          </div>
